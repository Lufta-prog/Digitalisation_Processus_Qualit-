...
class DashboardModel {
    ...
    public function getPerformanceData($period = 'month', $businessUnitId = null, $projectId = null, $activityId = null, $customerId = null) {
        ...
        $query = "SELECT 
                DATE_FORMAT(ds.Real_Date, '{$periodFormat}') as period,
                COUNT(CASE WHEN dv.FTR_Segula = 'OK' THEN 1 END) as ftr_segula_ok,
                COUNT(CASE WHEN dv.FTR_Segula = 'NOK' THEN 1 END) as ftr_segula_nok,
                COUNT(CASE WHEN dv.FTR_Customer = 'OK' THEN 1 END) as ftr_customer_ok,
                COUNT(CASE WHEN dv.FTR_Customer = 'NOK' THEN 1 END) as ftr_customer_nok,
                COUNT(CASE WHEN dv.OTD_Segula = 'OK' THEN 1 END) as otd_segula_ok,
                COUNT(CASE WHEN dv.OTD_Segula = 'NOK' THEN 1 END) as otd_segula_nok,
                COUNT(CASE WHEN dv.OTD_Customer = 'OK' THEN 1 END) as otd_customer_ok,
                COUNT(CASE WHEN dv.OTD_Customer = 'NOK' THEN 1 END) as otd_customer_nok,
                COUNT(*) as total
            ...
            {$whereClause}
            AND ds.Real_Date IS NOT NULL
            GROUP BY period
            ORDER BY ds.Real_Date";
    ...
    public function getRecentDeliverables($limit = 20) {
        $query = "SELECT pd.ID_Row, pd.Description_Topic, ds.Real_Date, 
                        dv.FTR_Segula, dv.OTD_Segula, dv.FTR_Customer, dv.OTD_Customer,
                 ...
                 WHERE pd.Livrable = 1 AND ds.Real_Date IS NOT NULL
                 ORDER BY ds.Real_Date DESC
                 LIMIT :limit";
        $stmt = $this->db->prepare($query);
